...
>
> **[Sub-fase 7.B++ CONCLUÍDA em 2026-05-26]** — Parser passou a extrair `descricao_projeto` do HEADER (concatenação de `$TITLE/$SUBJECT/$KEYWORDS/$COMMENTS/$AUTHOR`, deduplicada e separada por `" — "`). A descrição preenche o campo "Descrição" do produto **apenas se vazio** (mesma política do nome). Os tokens da descrição também alimentam o scoring de insumo do `DxfSugestaoInsumoService` com peso **0.5** — reforçam quando a camada tem nome genérico, sem dominar a evidência principal. Novo `NovoInsumoModal` (frontend) compacto, com os 8 campos obrigatórios do `CreateInsumoDto` + lógica de consumo, abre via botão "Cadastrar novo" por camada no `DxfRevisaoCard`. Ao criar, o insumo é atrelado imediatamente ao produto **e** as sugestões são recarregadas via `GET /dxf-extraido`. `versao_parser` bumpado para `7.B-1.1`. `MTEXT/TEXT` de entidades **não** capturados (decisão registrada para evitar poluir descrição com cotas). Detalhes na seção 4.18 do HANDOFF.
>
