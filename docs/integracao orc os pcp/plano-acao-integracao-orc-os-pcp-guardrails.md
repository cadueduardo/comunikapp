...
## Fase 1 — Fundamentos de OS e integraçao inicial (6–8 semanas)
...
- **Backlog**
  ...
  2. [ ] Implementar migracao batches de OS existentes para novo formato com rollback seguro.
  ...
  5. [ ] Adicionar campo `profundidade` ao model ProdutoOrcamento (Decimal opcional).
  6. [ ] Adicionar campos de aprovacao tecnica ao model OrdemServico:
  ...
  7. [ ] Adicionar campos de agendamento de instalacao ao model OrdemServico:
     - `data_instalacao_agendada` (DateTime opcional)
     - `observacoes_instalacao` (Text opcional)
  8. [ ] Criar helpers de transformacao:
  ...
  9. [ ] Criar template basico de impressao da OS em formato A4:
...
### **NOVO - Estrutura Completa para OS Direta/Interna (4-6 semanas)**
  10. [ ] **Schema Prisma - Estrutura Completa**:
  ...
  11. [ ] **DocumentCodeService - Numeração Diferenciada**:
...
## Fase 2 — MVP do PCP e workflows operacionais (5–6 semanas)
...
- **Backlog**
  1. [ ] Implementar fluxo de aprovacao tecnica:
  ...
  2. [ ] Implementar agendamento de instalacao:
  ...
  3. [ ] Modelar entidades de workflow no Prisma respeitando limites de tamanho de service/controller.
  4. [ ] Criar services e controllers Nest para instanciar workflows na criacao de OS e gerenciar transicoes.
  5. [ ] Implementar apontamentos (inicio/pausa/conclusao/refugo) acionando reservas/baixas de estoque quando aplicavel.
  6. [ ] Configurar notificacoes e webhooks por etapa/atraso, com templates multi-tenant.
  7. [ ] Desenvolver UI Kanban reutilizando componentes padrao, com estados vazio/loading e filtros de busca:
  ...
  8. [ ] Criar tela de detalhes da OS mostrando:
  ...
  9. [ ] Aprimorar template de impressao com checklists operacionais:
  ...
  10. [ ] Garantir testes unitarios/e2e nos novos fluxos e documentar endpoints OpenAPI.
...
## Apêndice A — Prompt‑mestre (System Message para Cursor)

```
You are a software engineering agent working on the project “Orçamento → OS → PCP”.
Your single source of truth is the document: Plano de ação para integração Orçamento → OS → PCP (abreviado “PLANO”).
All work MUST comply with the PLANO. If a requirement seems missing/ambíguo, STOP and ask structured questions before coding.

NON-NEGOTIABLE INVARIANTS (hard rules)
1) Escopo e modularidade
   - Não tocar em módulos fora do escopo da tarefa atual.
   - Reutilizar serviços existentes: DocumentCodeService, ValidacaoEstoqueService, WorkflowService.
   - Manter UX com componentes padrão e validações preventivas.

2) Dados e numeração
   - OS-AAAA-NNN é o código único de rastreamento (gerado pelo DocumentCodeService). PROIBIDO criar campo “TRK-OS”.
   - Estrutura de OS direta/interna = mesma da OS derivada do orçamento (ItemInsumoOS, ItemMaquinaOS, ItemFuncaoOS, ItemServicoManualOS, ItemCustoIndiretoOS).
   - Evitar campos duplicados: usar insumos/serviços/máquinas existentes; derivar “materiais principais”, “acabamentos”, “instalação”, “tipo de impressão” conforme definido no PLANO.

3) Prisma e migrações (SAFE MODE)
   - Apenas migrações aditivas/compatíveis; PROIBIDO remover/renomear colunas/tabelas existentes sem plano de migração em 2 etapas.
   - Toda migração deve incluir: script up/down seguro, dados seed mínimos, teste de rollback e teste e2e passando.
   - Após alterar schema: `prisma migrate` + `npx prisma generate` + rodar toda suíte de testes.

4) Qualidade e entrega
   - OpenAPI atualizado automaticamente em cada PR.
   - Cobertura de testes ≥ 80% para o que foi modificado (unit + e2e).
   - Health checks e logs adicionados quando novos serviços forem criados.
   - Proibir merges com testes quebrados ou diffs de OpenAPI não regenerados.

WORKFLOW DE TAREFA (para cada issue/objetivo)
A) “READ & PLAN”
   - Leia o PLANO e cite as seções relevantes.
   - Produza um Mini-ADR (Architecture Decision Record) de até 15 linhas: problema, opções consideradas, decisão, por quê.
   - Liste artefatos a entregar (código, migração, testes, docs).

B) “GUARDRAILS”
   - Enumere INVARIANTES afetadas e como serão preservadas.
   - Informe riscos e mitigação (ex.: impacto em estoque/compras, numeração, multi-tenant).

C) “IMPLEMENT”
   - Commits atômicos e sem efeitos colaterais: “feat(os): …”, “chore(prisma): …”, “test(e2e): …”.
   - Adote feature flags quando houver comportamento novo possivelmente disruptivo.

D) “VERIFY”
   - Rode Test Plan explícito (unit/e2e listados abaixo).
   - Gere e anexe o diff do OpenAPI.
   - Execute o “PR Checklist” (abaixo) e cole o resultado no PR.

E) “HANDOFF”
   - Atualize documentação (README/Guia de uso) e referência a endpoints.
   - Adicione exemplos de request/response reais.

TEST PLAN MÍNIMO (deve existir antes do merge)
- ValidacaoEstoqueService: OK / insuficiente / fornecedor inativo.
- Regras condicionais de OS: COMERCIAL vs INTERNA (cliente_id obrigatório vs centro_custo/justificativa obrigatórios).
- Numeração diferenciada: OS, OSD, OSI, OSE, OSM.
- Helpers de transformação: prazo em dias, data de entrega, materiais principais, acabamentos, tipo de impressão.
- Acesso/permits: somente perfis corretos aprovam “aprovação técnica” e “alçadas interna”.

FORBIDDEN SHORTCUTS
- “Codar rápido” sem checklist ou sem testes.
- Mudar schema com quebra silenciosa.
- Incluir campos duplicados onde o PLANO pede derivação/calculada.
- Alterar numeração ou serviços compartilhados sem ADR e testes.

OUTPUTS OBRIGATÓRIOS POR TAREFA
- Mini-ADR (markdown).
- PR com: link para seção do PLANO, evidências de testes, screenshot do OpenAPI diff, migração Prisma e scripts de rollback, notas de risco/mitigação.
```

---

## Apêndice B — Template de **PR Checklist**

- [ ] Referenciei a(s) seção(ões) do **PLANO** que embasam esta mudança.
- [ ] Mantive invariantes de dados (numeração OS-AAAA-NNN; nada de campo TRK-OS).
- [ ] Reaproveitei serviços (DocumentCodeService/ValidacaoEstoqueService/WorkflowService).
- [ ] Migração Prisma é **aditiva** e tem rollback testado (instruções incluídas).
- [ ] `npx prisma generate` rodado; tipos validados no backend e frontend.
- [ ] OpenAPI regenerado e diff anexado.
- [ ] Cobertura ≥ 80% nos arquivos alterados (unit + e2e).
- [ ] Test Plan executado com evidências (logs/prints).
- [ ] Riscos & mitigação documentados (ex.: estoque, multi-tenant, performance).
- [ ] Docs/README atualizados.

> Sugestão: salvar este bloco como `.github/pull_request_template.md` para aplicar a todos os PRs.

---

## Apêndice C — **Safe Prisma Migration** (passo a passo)

1. Criar migration **aditiva** (novas colunas/tabelas; campos `NULL` por padrão quando aplicável).
2. Popular dados default/seed via script **idempotente**.
3. Rodar `prisma migrate dev` + `npx prisma generate` e commit separado `chore(prisma): migration`.
4. Escrever teste de integração que cobre CRUD e leitura legada.
5. Prover script de **rollback** (down) validado em homologação.
6. Marcar PR com label “schema-change”.

---

## Apêndice D — **Gates por fase** (aceite objetivo)

### Fase 1 — Fundamentos de OS
- [ ] Numeração OS via DocumentCodeService ativa + histórico migrado OK.
- [ ] OS criada automaticamente após orçamento aprovado, **preservando insumos**.
- [ ] Validações robustas em `validarDadosOS` (estoque, status, cliente/loja) com retorno estruturado ao frontend.
- [ ] Campos novos: `profundidade`, `aprovacao_tecnica_*`, `data_instalacao_agendada`, `observacoes_instalacao`.
- [ ] Estrutura completa para OS direta/interna (tabelas Item*OS) criada e com CRUDs.
- [ ] Interface adaptativa (COMERCIAL vs INTERNA) + validações condicionais.
- [ ] Template básico de impressão `/os/:id/imprimir` entregue.
- [ ] Testes e2e: criação OS derivada; OS interna com alçada; validação de estoque (3 cenários).

### Fase 2 — MVP do PCP
- [ ] Workflow de aprovação técnica (API, permissões, status, notificações).
- [ ] Entidades de workflow ligadas à OS (instâncias/etapas/checklists/apontamentos).
- [ ] Kanban simplificado com badges de SLA e materiais.
- [ ] Agendamento de instalação (condicional por “instalação” em serviços_manuais).
- [ ] Logs de movimentação (tipo, usuário, IP/user-agent) + webhooks.
- [ ] Testes e2e cobrindo aprova/rejeita técnica, transições de estado e permissões.
- [ ] Métricas iniciais: lead time de OS e taxa de retrabalho (coleta no código).

---

## Apêndice E — Template de Q&A (antes de codar)

```
Q&A PARA CLARIFICAÇÃO
1) Objetivo da tarefa:
2) Seções do PLANO relacionadas:
3) Dados/tabelas afetados:
4) Invariantes potencialmente tocados e como serão preservados:
5) Riscos e mitigação:
6) Saídas/Artefatos:
7) Validação de sucesso (testes/telemetria):
```
