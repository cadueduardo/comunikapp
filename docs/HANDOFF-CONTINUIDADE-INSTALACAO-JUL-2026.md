...
### 8.3 Promoção automática para Em andamento (jul/2026)

`InstalacaoExecucaoSyncService` reconcilia status ao abrir o painel (`obterPainelOs`) e após mudanças:

1. **Alocação 100%** (todos os itens instaláveis sem saldo pendente) → todos os lotes `AGUARDANDO` viram `EM_ANDAMENTO` e a OS recebe `status_instalacao_os = EM_ANDAMENTO`.
2. **Ocorrência vinculada a um lote** (evidência de atendimento) → aquele lote `AGUARDANDO` vira `EM_ANDAMENTO`.
