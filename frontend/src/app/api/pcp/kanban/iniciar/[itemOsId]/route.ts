...
export async function POST(
  ...
  try {
    ...
    console.log('🔍 API Route - Iniciando produção do item:', itemOsId);
    
    const response = await fetch(
      buildApiUrl(`/pcp/kanban/iniciar/${itemOsId}`),
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.message || 'Erro ao iniciar produção' },
    ...
    console.error('Erro na API route /api/pcp/kanban/iniciar/[itemOsId]:', error);
    return NextResponse.json(
