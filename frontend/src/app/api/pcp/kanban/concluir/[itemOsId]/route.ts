...
export async function POST(
  ...
  try {
    ...
    console.log('🔍 API Route - Concluindo etapa do item:', itemOsId);
    
    const response = await fetch(
      buildApiUrl(`/pcp/kanban/concluir/${itemOsId}`),
      {
        method: 'POST',
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.message || 'Erro ao concluir etapa' },
    ...
    console.error('Erro na API route /api/pcp/kanban/concluir/[itemOsId]:', error);
    return NextResponse.json(
