...
export default function EditarRegraPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();
  const resolvedParams = use(params);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [regra, setRegra] = useState<RegraValidacao | null>(null);
  ...
  useEffect(() => {
    const loadRegra = async () => {
      try {
        const response = await fetch(`/api/configuracoes/regras-validacao/${resolvedParams.id}`);
        const data = await response.json();
        
        if (data.success && data.data) {
          setRegra(data.data);
          
          // Parse condicoes e acoes se vierem como string
          const condicoes = typeof data.data.condicoes === 'string' 
            ? JSON.parse(data.data.condicoes) 
            : data.data.condicoes;
          
          const acoes = typeof data.data.acoes === 'string' 
            ? JSON.parse(data.data.acoes) 
            : data.data.acoes;
          ...
          setFormData({
            ...
            loja_id: data.data.loja_id || '',
            condicoes: condicoes || formData.condicoes,
            acoes: acoes || formData.acoes
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Erro ao atualizar regra');
      }
    } catch (error) {
      ...
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Carregando dados da regra</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Regra não encontrada</h1>
            <p className="text-gray-600">A regra solicitada não foi encontrada</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Editar Regra de Validação</h1>
          <p className="text-gray-600">Modifique a regra: {regra.nome}</p>
        </div>
      </div>
