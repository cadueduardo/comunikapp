...
export function maskDocumentInput(value: string, tipoPessoa: 'fisica' | 'juridica'): string {
  const digits = stripDocument(value).slice(0, tipoPessoa === 'fisica' ? CPF_LENGTH : CNPJ_LENGTH);
