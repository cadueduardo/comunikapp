...
export class ValidacaoV2Service {
  ...
  private validarCamposObrigatorios(dados: any, criacao: boolean = true): void {
    const camposObrigatorios = ['titulo', 'cliente_id', 'tipo', 'prioridade'];
  ...
  private validarProduto(produto: any): void {
    if (!produto.nome || typeof produto.nome !== 'string') {
      throw new BadRequestException('Produto deve ter um nome válido');
    }
    ...
    if (!produto.unidade || typeof produto.unidade !== 'string') {
