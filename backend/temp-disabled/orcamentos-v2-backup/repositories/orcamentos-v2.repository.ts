...
export class OrcamentosV2Repository {
  ...
  async buscarEstatisticas(filtros?: {
    ...
    try {
      ...
      return {
        ...
        valor_total: valorTotal._sum.valor_total || 0,
        valor_medio: valorMedio._avg.valor_total || 0,
  ...
  private async buscarOrcamentosPorStatus(where: any): Promise<Record<string, number>> {
    const resultado = await this.prisma.orcamento.groupBy({
      by: ['status'],
      where,
      _count: { status: true },
    });
  ...
  private async buscarOrcamentosPorTipo(where: any): Promise<Record<string, number>> {
    ...
    const distribuicao: Record<string, number> = {};
    resultado.forEach(item => {
      distribuicao[item.tipo] = item._count.tipo;
    });
  ...
  private transformarOrcamento(orcamento: any): OrcamentoCompleto {
    return {
      ...
      data_entrega_prevista: orcamento.data_entrega_prevista,
      ...
      links_publicos: orcamento.links_publicos || [],
      mensagens_chat: orcamento.mensagens_chat || [],
