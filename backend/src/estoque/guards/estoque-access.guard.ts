...
export class EstoqueAccessGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!request.estoque) {
      throw new UnauthorizedException('Contexto de estoque não encontrado - middleware necessário');
    }

    // 2. VERIFICAÇÃO DE LOJA (MULTI-TENANT)
    if (!request.estoque.lojaId) {
      throw new UnauthorizedException('lojaId é obrigatório para acesso ao estoque');
    }

    // 3. VERIFICAÇÃO DE USUÁRIO AUTENTICADO
    if (!request.estoque.usuarioId) {
      throw new UnauthorizedException('usuarioId é obrigatório para operações de estoque');
    }

    // 4. VERIFICAÇÃO DE MÓDULO ATIVO
    const moduleEnabled = this.configService.get('ESTOQUE_MODULE_ENABLED', 'true') === 'true';
    if (!moduleEnabled) {
    ...
    const hasPermission = userRoles.some(role => allowedRoles.includes(role.trim()));
    if (!hasPermission) {
