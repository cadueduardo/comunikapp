...
export class TransferenciasController {
  ...
  async criarTransferencia(@GetLoja() loja: Loja, @Body() data: any) {
    ...
    try {
      if (!data.itemId || !data.localizacaoOrigemId || !data.localizacaoDestinoId || !data.quantidade) {
        throw new BadRequestException('Dados obrigatórios: itemId, localizacaoOrigemId, localizacaoDestinoId, quantidade');
      }

      if (data.localizacaoOrigemId === data.localizacaoDestinoId) {
        throw new BadRequestException('Localização de origem e destino não podem ser iguais');
      }

      const context = { lojaId: loja.id };
      const transferencia = await this.estoqueService.criarTransferencia(context, data);
  ...
  @ApiQuery({ name: 'itemId', required: false, description: 'Filtrar por item de estoque' })
  ...
  async listarTransferencias(@GetLoja() loja: Loja, @Query() query: any) {
    ...
    try {
      const context = { lojaId: loja.id };
      const transferencias = await this.estoqueService.listarTransferencias(context, query);
  ...
  @ApiResponse({ status: 200, description: 'Transferência encontrada com sucesso' })
  async buscarTransferencia(@GetLoja() loja: Loja, @Param('id') id: string) {
    ...
    try {
      const context = { lojaId: loja.id };
      const transferencia = await this.estoqueService.buscarTransferenciaPorId(context, id);
  ...
  @ApiResponse({ status: 200, description: 'Histórico de transferências retornado' })
  async historicoTransferenciasItem(@GetLoja() loja: Loja, @Param('itemId') itemId: string) {
    this.logger.log(`🔄 Buscando histórico de transferências do item ${itemId} para loja: ${loja.id}`);
    
    try {
      const context = { lojaId: loja.id };
      const historico = await this.estoqueService.historicoTransferenciasItem(context, itemId);
