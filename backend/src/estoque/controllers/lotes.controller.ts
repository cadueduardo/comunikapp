...
export class LotesController {
  ...
  @ApiQuery({ name: 'estoqueId', required: false, description: 'Filtrar por item de estoque' })
  ...
  @ApiParam({ name: 'id', description: 'ID do lote' })
  async atualizarLote(@GetLoja() loja: Loja, @Param('id') id: string, @Body() data: any) {
  ...
  @ApiResponse({ status: 200, description: 'Lotes próximos do vencimento retornados' })
  @ApiQuery({ name: 'dias', required: false, description: 'Dias para vencimento (padrão: 30)' })
  async lotesProximosVencimento(@GetLoja() loja: Loja, @Query('dias') dias: string = '30') {
    this.logger.log(`📦 Buscando lotes próximos do vencimento para loja: ${loja.id}`);
    
    try {
      const context = { lojaId: loja.id };
      const diasNum = parseInt(dias) || 30;
      const lotes = await this.estoqueService.lotesProximosVencimento(context, diasNum);
  ...
  async consumirLote(
    ...
    try {
      ...
      const context = { lojaId: loja.id };
      const resultado = await this.estoqueService.consumirLote(context, id, data.quantidade);
