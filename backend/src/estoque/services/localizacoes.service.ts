...
export class LocalizacoesService {
  ...
  async criarLocalizacao(context: IEstoqueContext, data: any) {
    ...
    this.logger.debug(
      `📍 [LocalizacoesService] criarLocalizacao loja=${context.lojaId}`,
    );
    ...
    if (existe?.length) {
      throw new BadRequestException(
        'Código de localização já existe nesta loja',
      );
    }
    ...
    if (!columns.length)
      throw new BadRequestException(
        'Estrutura da tabela estoque_localizacoes não encontrada.',
      );
  ...
  async listarLocalizacoes(context: IEstoqueContext, query: any = {}) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📍 [LocalizacoesService] listarLocalizacoes loja=${context.lojaId}`,
    );
    const whereParts: string[] = ['l.lojaId = ?'];
    const params: any[] = [context.lojaId];
    if (query?.deposito) {
      whereParts.push('l.deposito LIKE ?');
      params.push(`%${String(query.deposito)}%`);
    }
  ...
  async buscarLocalizacaoPorId(context: IEstoqueContext, id: string) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📍 [LocalizacoesService] buscarLocalizacaoPorId id=${id} loja=${context.lojaId}`,
    );
    const tableName = 'estoque_localizacoes';
    const rows: any[] = await this.prisma.$queryRawUnsafe(
      `SELECT 
  ...
  async atualizarLocalizacao(context: IEstoqueContext, id: string, data: any) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📍 [LocalizacoesService] atualizarLocalizacao id=${id} loja=${context.lojaId}`,
    );
    ...
    for (const key of Object.keys(data || {})) {
      ...
      if (col && existing.has(col)) {
        let value = data[key];
        if (key === 'codigo' && typeof value === 'string')
  ...
  async desativarLocalizacao(context: IEstoqueContext, id: string) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📍 [LocalizacoesService] desativarLocalizacao id=${id} loja=${context.lojaId}`,
    );
  ...
  async verificarLocalizacaoExclusao(context: IEstoqueContext, id: string) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📍 [LocalizacoesService] verificarLocalizacaoExclusao id=${id} loja=${context.lojaId}`,
    );
  ...
  async excluirLocalizacao(context: IEstoqueContext, id: string) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📍 [LocalizacoesService] excluirLocalizacao id=${id} loja=${context.lojaId}`,
    );
