...
export class ItensEstoqueService {
  ...
  async criarItemEstoque(context: IEstoqueContext, data: any) {
    ...
    this.logger.debug(`🧩 [ItensEstoqueService] criarItemEstoque loja=${context.lojaId}`);
    ...
    const base = baseNome.toString().toUpperCase().replace(/[^A-Z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 12) || 'ITEM';
    const suffix = Math.random().toString(36).slice(2, 7).toUpperCase();
    const codigo = (typeof data.codigo === 'string' && data.codigo.trim()) ? data.codigo.trim() : `${base}-${suffix}`;
    add('codigo', codigo);
    add('nome', String(data.nome).trim());
    add('descricao', data.descricao || null);
    add('quantidadeAtual', data.quantidadeAtual ?? 0); add('quantidade', data.quantidadeAtual ?? 0);
    add('quantidadeReservada', data.quantidadeReservada ?? 0);
    add('estoqueMinimo', data.estoqueMinimo ?? 0); add('estoque_minimo', data.estoqueMinimo ?? 0);
    add('estoqueMaximo', data.estoqueMaximo ?? null); add('estoque_maximo', data.estoqueMaximo ?? null);
    add('unidadeMedida', data.unidadeMedida); add('unidade_medida', data.unidadeMedida);
    add('precoUnitario', data.precoUnitario ?? 0); add('preco_unitario', data.precoUnitario ?? 0);
    add('codigoBarras', data.codigoBarras || null); add('codigo_barras', data.codigoBarras || null);
    add('lote', data.lote || null);
    add('dataValidade', data.dataValidade || null); add('data_validade', data.dataValidade || null);
    add('ativo', data.ativo !== undefined ? !!data.ativo : true);
    add('observacoes', data.observacoes || null);

    if (columns.length === 0) throw new BadRequestException(`Estrutura da tabela ${tableName} não possui colunas compatíveis.`);
    const placeholders = columns.map(() => '?').join(', ');
    await this.prisma.$executeRawUnsafe(`INSERT INTO ${tableName} (${columns.join(', ')}) VALUES (${placeholders})`, ...values);
    const selected: any[] = await this.prisma.$queryRawUnsafe(`SELECT * FROM ${tableName} WHERE id = ? LIMIT 1`, generatedId);
    return selected?.[0] || { id: generatedId, ...data, lojaId: context.lojaId };
  }
  ...
  async listarItensEstoque(context: IEstoqueContext, query: any = {}) {
    ...
    if (items.length === 0) {
      const mockData = [
        {
          id: 'item-001', insumoId: 'insumo-001', insumoNome: 'Item Exemplo', localizacaoId: 'loc-001', localizacaoCodigo: 'A1', quantidadeAtual: 10, quantidadeReservada: 0, estoqueMinimo: 1, estoqueMaximo: 100, unidadeCompra: 'UN', valorUnitario: 0, dataUltimaMov: new Date(), createdAt: new Date(),
        },
      ];
      return { data: mockData, total: mockData.length, page: query.page || 1, limit: query.limit || 20 } as any;
    }

    return { data: items, total: items.length, page: query.page || 1, limit: query.limit || 20 } as any;
  }
  ...
  async buscarItemEstoquePorId(context: IEstoqueContext, id: string) {
    ...
    if (items.length === 0) throw new BadRequestException('Item de estoque não encontrado');
  ...
  async atualizarItemEstoque(context: IEstoqueContext, id: string, data: any) {
    ...
    const map: Record<string, string> = {
      ...
      quantidadeAtual: existing.has('quantidadeAtual') ? 'quantidadeAtual' : (existing.has('quantidade') ? 'quantidade' : 'quantidade'),
    ...
    Object.keys(data || {}).forEach((k) => {
      const col = map[k];
      if (col && data[k] !== undefined) { updates.push(`${col} = ?`); params.push(data[k]); }
    });
