...
export class LotesService {
  ...
  async criarLote(context: IEstoqueContext, data: any) {
    ...
    if (!itensTable)
      throw new BadRequestException(
        'Estrutura de itens de estoque não encontrada',
      );
    ...
    if (!lotesTable)
      throw new BadRequestException('Estrutura de lotes não encontrada');
    ...
    const joinInsumoCol = itensCols.has('insumo_id')
      ...
      : itensCols.has('insumoId')
        ? 'insumoId'
        : null;
    const joinLocalizacaoCol = itensCols.has('localizacaoId')
      ? 'localizacaoId'
      : 'localizacao_id';
  ...
  async buscarLotePorId(context: IEstoqueContext, id: string) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📦 [LotesService] buscarLotePorId id=${id} loja=${context.lojaId}`,
    );
  ...
  async atualizarLote(context: IEstoqueContext, id: string, data: any) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📦 [LotesService] atualizarLote id=${id} loja=${context.lojaId}`,
    );
    ...
    if (data.numeroLote !== undefined) {
      ...
      params.push(data.numeroLote || null);
    }
    if (data.dataFabricacao !== undefined) {
      updates.push('data_fabricacao = ?');
      params.push(data.dataFabricacao || null);
    }
    if (data.dataValidade !== undefined) {
      updates.push('data_validade = ?');
      params.push(data.dataValidade || null);
    }
    if (data.quantidadeLote !== undefined) {
      updates.push('quantidade_lote = ?');
      params.push(Number(data.quantidadeLote));
    }
  ...
  async excluirLote(context: IEstoqueContext, id: string) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📦 [LotesService] excluirLote id=${id} loja=${context.lojaId}`,
    );
    const lote = await this.buscarLotePorId(context, id);
    if (!lote) throw new BadRequestException('Lote não encontrado');
    await this.prisma.$executeRawUnsafe(
      `DELETE FROM estoque_lotes WHERE id = ? AND loja_id = ?`,
      id,
      context.lojaId,
    );
  ...
  async lotesProximosVencimento(context: IEstoqueContext, dias: number = 30) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📦 [LotesService] lotesProximosVencimento dias=${dias} loja=${context.lojaId}`,
    );
  ...
  async consumirLote(context: IEstoqueContext, id: string, quantidade: number) {
    if (!context?.lojaId) throw new BadRequestException('lojaId é obrigatório');
    this.logger.debug(
      `📦 [LotesService] consumirLote id=${id} qtd=${quantidade} loja=${context.lojaId}`,
    );
    const lote = await this.buscarLotePorId(context, id);
    if (!lote) throw new BadRequestException('Lote não encontrado');
    if (lote.status !== 'ATIVO')
      throw new BadRequestException('Lote não está ativo');
    if (
      Number(lote.quantidade_lote ?? lote.quantidadeLote) < Number(quantidade)
    )
      throw new BadRequestException('Quantidade insuficiente no lote');
