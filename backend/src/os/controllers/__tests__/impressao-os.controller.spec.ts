...
describe('ImpressaoOSController', () => {
  ...
  describe('imprimirOS', () => {
    it('deve gerar template HTML para impressão', async () => {
      ...
      expect(mockRes.setHeader).toHaveBeenCalledWith('Content-Disposition', 'inline; filename="OS-OS-2024-001.html"');
    ...
    it('deve lançar HttpException em caso de erro', async () => {
      ...
      mockImpressaoOSService.gerarDadosImpressao.mockRejectedValue(new Error(errorMessage));
  ...
  describe('previewImpressao', () => {
    it('deve gerar preview do template', async () => {
      ...
      expect(mockRes.setHeader).toHaveBeenCalledWith('Content-Type', 'text/html; charset=utf-8');
      expect(mockRes.send).toHaveBeenCalledWith(expect.stringContaining('preview-mode'));
    });
    ...
    it('deve lançar HttpException em caso de erro no preview', async () => {
      ...
      await expect(controller.previewImpressao(osId, 'true', 'true', 'true', mockRes))
        .rejects.toThrow(HttpException);
  ...
  describe('obterDadosImpressao', () => {
    it('deve retornar dados estruturados da OS', async () => {
      ...
      const resultado = await controller.obterDadosImpressao(osId, 'true', 'true', 'true');
    ...
    it('deve lançar HttpException em caso de erro', async () => {
      ...
      await expect(controller.obterDadosImpressao(osId, 'true', 'true', 'true'))
        .rejects.toThrow(HttpException);
