...
export class OSPrazoController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Data inválida ou OS não encontrada',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Status do prazo consultado com sucesso',
  })
  async consultarStatusPrazo(@Param('id') osId: string, @Request() req: any) {
    try {
      const lojaId = req.user.loja_id;
      const status = await this.osPrazoService.consultarStatusPrazo(
        osId,
        lojaId,
      );
