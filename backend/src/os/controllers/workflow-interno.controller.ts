...
export class WorkflowInternoController {
  ...
  @ApiOperation({ summary: 'Transicionar OS para próximo estado do workflow interno' })
  ...
  @ApiOperation({ summary: 'Iniciar produção da OS (após aprovação orçamentária)' })
  @ApiResponse({ status: 200, description: 'Produção iniciada com sucesso' })
  @ApiResponse({ status: 400, description: 'OS não está aprovada orçamentariamente' })
