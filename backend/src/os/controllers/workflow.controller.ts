/**
 * Controller para gestao de Workflows
 * Limite: <= 200 linhas conforme premissas
 * Endpoints: CRUD workflows, validacao de etapas
 */
...
export class WorkflowController {
  ...
  async criarPadrao(@Request() req: AuthenticatedRequest) {
    ...
    return {
      success: true,
      data: workflow,
      message: 'Workflow padrao criado com sucesso',
      timestamp: new Date().toISOString(),
    };
  }
  ...
  @ApiOperation({ summary: 'Validar transicao entre etapas' })
  @ApiResponse({ status: 200, description: 'Validacao realizada' })
  async validarTransicao(
    @Param('id') id: string,
    @Body() body: { etapa_origem: string; etapa_destino: string },
    @Request() req: AuthenticatedRequest,
  ) {
    ...
    return {
      success: true,
      data: validacao,
      message: validacao.valida ? 'Transicao valida' : validacao.motivo,
      timestamp: new Date().toISOString(),
    };
  }
}
