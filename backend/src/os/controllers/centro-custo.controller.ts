...
export class CentroCustoController {
  ...
  @ApiResponse({ status: 200, description: 'Lista de centros de custo obtida com sucesso' })
  async listarCentrosCusto(@Param('lojaId') lojaId: string) {
    const centrosCusto = await this.centroCustoService.listarCentrosCusto(lojaId);
  ...
  @ApiResponse({ status: 400, description: 'Orçamento insuficiente ou dados inválidos' })
  ...
  @ApiResponse({ status: 400, description: 'Reserva não encontrada ou dados inválidos' })
  ...
  async obterRelatorioConsumoDepartamento(
    ...
    const relatorios = await this.centroCustoService.obterRelatorioConsumoDepartamento(
      lojaId,
      departamento,
      periodoInicio,
      periodoFim
  ...
  @ApiOperation({ summary: 'Obter relatório consolidado de todos os departamentos' })
  @ApiResponse({ status: 200, description: 'Relatório consolidado obtido com sucesso' })
  @ApiQuery({ name: 'loja_id', required: true, type: String })
  @ApiQuery({ name: 'periodo_inicio', required: false, type: Date })
  @ApiQuery({ name: 'periodo_fim', required: false, type: Date })
  ...
  async enviarAlertasLimiteGastos(
    ...
    await this.centroCustoService.enviarAlertasLimiteGastos(body.loja_id, body.alertas);
  ...
  @ApiOperation({ summary: 'Obter histórico de movimentações do centro de custo' })
  ...
  async obterDashboardCentroCusto(@Param('lojaId') lojaId: string) {
    ...
    return {
      ...
      resumo: {
        ...
        centros_ativos: centrosCusto.filter(cc => cc.status === 'ATIVO').length,
        total_alertas: alertas.length,
        alertas_criticos: alertas.filter(a => a.prioridade === 'CRITICA').length
      },
