...
export class AlcadasOrcamentoController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Configurações de alçadas obtidas com sucesso',
  })
  ...
  @ApiOperation({
    summary: 'Validar se usuário pode aprovar valor automaticamente',
  })
  ...
  @ApiOperation({
    summary: 'Identificar aprovador necessário baseado no valor',
  })
  @ApiResponse({
    status: 200,
    description: 'Aprovador identificado com sucesso',
  })
  async identificarAprovadorNecessario(
    @Body() body: { valor_estimado: number },
  ) {
    const aprovadorNecessario =
      await this.alcadasOrcamentoService.identificarAprovadorNecessario(
        body.valor_estimado,
      );
  ...
  async processarAprovacaoAutomatica(
    ...
    const resultado =
      await this.alcadasOrcamentoService.processarAprovacaoAutomatica(
        body.os_id,
        body.valor_estimado,
        body.centro_custo,
        body.loja_id,
  ...
  async obterRelatorioConsumoDepartamento(
    ...
    const relatorio =
      await this.alcadasOrcamentoService.obterRelatorioConsumoDepartamento(
        lojaId,
        periodoInicio,
        periodoFim,
      );
