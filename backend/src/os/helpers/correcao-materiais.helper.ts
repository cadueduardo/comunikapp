...
export class CorrecaoMateriaisHelper {
  ...
  static corrigirInsumosCalculados(
    ...
    if (!insumosCalculados || !Array.isArray(insumosCalculados) || insumosCalculados.length === 0) {
      return [];
    }
    ...
    return insumosCalculados.map(insumo => {
      ...
      if (precisaCorrecao) {
        ...
        insumoCorrigido.quantidade_necessaria = insumo.quantidade_necessaria * quantidadeProduto;
        insumoCorrigido.custo_total = insumoCorrigido.quantidade_necessaria * insumo.custo_unitario;
  ...
  static validarCorrecao(
    ...
    insumosOriginais.forEach((original, index) => {
      ...
      if (deveMultiplicar) {
        const quantidadeEsperada = original.quantidade_necessaria * quantidadeProduto;
        const custoTotalEsperado = quantidadeEsperada * original.custo_unitario;

        if (Math.abs(corrigido.quantidade_necessaria - quantidadeEsperada) > 0.01) {
          erros.push(`Quantidade incorreta para ${original.nome}: esperado ${quantidadeEsperada}, obtido ${corrigido.quantidade_necessaria}`);
        }

        if (Math.abs(corrigido.custo_total - custoTotalEsperado) > 0.01) {
          erros.push(`Custo total incorreto para ${original.nome}: esperado ${custoTotalEsperado}, obtido ${corrigido.custo_total}`);
        }
      } else {
        // Para materiais m², não deve haver alteração
        if (original.quantidade_necessaria !== corrigido.quantidade_necessaria) {
          erros.push(`Quantidade alterada incorretamente para ${original.nome} (m²): esperado ${original.quantidade_necessaria}, obtido ${corrigido.quantidade_necessaria}`);
        }

        if (original.custo_total !== corrigido.custo_total) {
          erros.push(`Custo total alterado incorretamente para ${original.nome} (m²): esperado ${original.custo_total}, obtido ${corrigido.custo_total}`);
        }
      }
    });
