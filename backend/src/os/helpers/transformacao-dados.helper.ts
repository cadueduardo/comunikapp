...
export class TransformacaoDadosHelper {
   ...
   */
  static calcularPrazoProducaoDias(
    horasTotais: number,
    horasDiaUtil: number = 8,
  ): number {
   ...
   */
  static converterPrazoEntregaParaData(
    prazoEntrega: string,
    dataAbertura: Date,
  ): Date {
  ...
  static extrairMateriaisPrincipais(insumos: any[]): MaterialPrincipal[] {
    ...
    const insumosOrdenados = insumos
      ...
      .sort((a, b) => {
        const custoA =
          typeof a.custo_total === 'string'
            ? parseFloat(a.custo_total)
            : a.custo_total;
        const custoB =
          typeof b.custo_total === 'string'
            ? parseFloat(b.custo_total)
            : b.custo_total;
    ...
    return insumosOrdenados.slice(0, 3).map((insumo) => ({
      ...
      quantidade:
        typeof insumo.quantidade === 'string'
          ? parseFloat(insumo.quantidade)
          : insumo.quantidade,
      unidade: insumo.unidade || insumo.insumo?.unidade_uso || 'un',
      custo_total:
        typeof insumo.custo_total === 'string'
          ? parseFloat(insumo.custo_total)
          : insumo.custo_total,
    }));
  }
  ...
  static identificarTipoImpressao(maquinas: any[]): TipoImpressao | null {
    ...
    const confianca = Math.round(
      (contadores[tipoPredominante] / totalMaquinas) * 100,
    );

    return {
      tipo: tiposImpressao[tipoPredominante] || tipoPredominante,
      maquina:
        maquinas.find((m) => (m.tipo || m.maquina?.tipo) === tipoPredominante)
          ?.nome || 'Máquina',
      confianca,
    };
  }
  ...
  static extrairAcabamentos(servicosManuais: any[]): Acabamento[] {
    ...
    return servicosManuais
      .filter((servico) => {
        const categorias =
          servico.categorias || servico.servico?.categorias || [];
      ...
      .map((servico) => {
        ...
        return {
          ...
          custo_total:
            typeof servico.custo_total === 'string'
              ? parseFloat(servico.custo_total)
              : servico.custo_total,
        };
      });
  }
  ...
  static transformarDadosCompletos(dadosOrcamento: {
    ...
    const prazoProducaoDias = this.calcularPrazoProducaoDias(
      dadosOrcamento.horasProducao,
    );
    ...
    const materiaisPrincipais = this.extrairMateriaisPrincipais(
      dadosOrcamento.insumos,
    );

    // Identificar tipo de impressão
    const tipoImpressao = this.identificarTipoImpressao(
      dadosOrcamento.maquinas,
    );
    ...
    const instalacaoNecessaria = this.verificarInstalacaoNecessaria(
      dadosOrcamento.servicosManuais,
    );
  ...
  static formatarParaExibicao(dados: DadosTransformacao): {
    ...
    return {
      ...
      materiaisFormatados: dados.materiaisPrincipais.map(
        (m) => `${m.nome} (${m.quantidade} ${m.unidade})`,
      ),
