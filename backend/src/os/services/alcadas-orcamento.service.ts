...
export class AlcadasOrcamentoService {
  ...
  async podeAprovarAutomaticamente(
    ...
    try {
      ...
      if (
        valorEstimado <= config.limite_maximo &&
        config.pode_aprovar_automaticamente
      ) {
        return { pode: true };
      }
  ...
  async validarOrcamentoDisponivel(
    ...
    try {
      const validacao =
        await this.centroCustoService.validarOrcamentoDisponivel(
          centroCusto,
          valorSolicitado,
          lojaId,
        );
  ...
  async enviarNotificacaoAprovacao(
    notificacao: NotificacaoAprovacao,
  ): Promise<void> {
    try {
      this.logger.log(
        `Notificação enviada para ${notificacao.aprovador_id}: ${notificacao.mensagem}`,
      );
  ...
  async processarAprovacaoAutomatica(
    ...
    try {
      ...
      const podeAprovar = await this.podeAprovarAutomaticamente(
        usuario.funcao,
        valorEstimado,
      );
      if (!podeAprovar.pode) {
      ...
      const validacaoOrcamento = await this.validarOrcamentoDisponivel(
        centroCusto,
        valorEstimado,
        lojaId,
      );
      ...
      const reserva = await this.reservarOrcamento(
        centroCusto,
        valorEstimado,
        lojaId,
        osId,
      ...
      this.logger.log(
        `OS ${osId} aprovada automaticamente por ${usuario.funcao}`,
      );
