...
export class OSPrazoService {
  ...
  async definirPrazo(request: DefinirPrazoRequest) {
    ...
    const osAtualizada = await this.prisma.ordemServico.update({
      ...
      data: {
        ...
        motivo_modificacao:
          motivo || (isUpdate ? 'Prazo atualizado' : 'Prazo definido'),
      },
    });
   ...
   */
  async consultarStatusPrazo(
    osId: string,
    lojaId: string,
  ): Promise<StatusPrazoResponse> {
    const os = await this.prisma.ordemServico.findFirst({
      where: {
        id: osId,
        loja_id: lojaId,
    ...
    if (isRetroativa || isHoje) {
      ...
      mensagem = isRetroativa
        ? 'OS pronta para produção (prazo retroativo)'
        : 'OS pronta para produção (prazo hoje)';
    } else {
