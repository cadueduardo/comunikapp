...
export class OSDiretaInternaValidator {
  ...
  validarCamposOSInterna(dados: any): void {
    const camposObrigatorios = ['departamento_solicitante', 'centro_custo'];
    ...
    if (
      dados.centro_custo &&
      !this.validarFormatoCentroCusto(dados.centro_custo)
    ) {
      throw new BadRequestException(
  ...
  validarAprovacaoGerencial(dados: any): void {
    ...
    if (!this.validarStatusAprovacao(dados.aprovacao_gerencial)) {
      throw new BadRequestException('Status de aprovação gerencial inválido');
    }

    if (
      dados.aprovacao_gerencial === StatusAprovacao.APROVADA &&
      !dados.aprovacao_gerencial_por
    ) {
      throw new BadRequestException(
  ...
  validarCamposControle(dados: any): void {
    if (
      dados.versao !== undefined &&
      (!Number.isInteger(dados.versao) || dados.versao < 1)
    ) {
      throw new BadRequestException(
    ...
    if (
      dados.motivo_modificacao &&
      dados.motivo_modificacao.trim().length < 10
    ) {
      throw new BadRequestException(
   ...
   */
  validarPermissaoAprovacao(
    os: any,
    tipoAprovacao: 'tecnica' | 'gerencial',
  ): void {
