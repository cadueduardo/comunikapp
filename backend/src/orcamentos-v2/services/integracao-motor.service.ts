...
export class IntegracaoMotorService {
  ...
  async calcularOrcamentoCompleto(
    ...
    try {
      ...
      const resultadoMotor = await this.motorCalculoV2Service.executarCalculo(
        dadosMotor,
      );
  ...
  async validarOrcamento(
    ...
    try {
      ...
      const resultadoValidacao = await this.motorCalculoV2Service.validarContexto(dto);
  ...
  async calcularOrcamentosEmLote(
    ...
    try {
      ...
      for (const orc of orcamentos) {
        try {
          const resultado = await this.motorCalculoV2Service.calcularOrcamento(orc, lojaId);
  ...
  async obterEstatisticasMotor(lojaId: string): Promise<any> {
    ...
    try {
      ...
      const estatisticas = await this.motorCalculoV2Service.obterEstatisticas(lojaId);
  ...
  private prepararDadosParaMotor(dadosOrcamento: any, lojaId: string): any {
    ...
    const dadosMotor = {
      ...
      configuracoes: {
        margem_lucro: parseFloat(dadosOrcamento.margem_lucro_customizada || '30'),
    ...
    this.logger.log(`✅ Dados preparados para motor V2: ${dadosMotor.produtos.length} produtos`);
    return dadosMotor;
  }
  ...
  private prepararProdutosParaMotor(produtos: any[]): any[] {
    return produtos.map(produto => ({
      ...
      servicos: (produto.servicos_manuais || produto.servicos || []).map((servico: any) => {
