...
export class LinksV2Service {
  ...
  async acessarLinkPublico(
    ...
    try {
      ...
      return {
        ...
        orcamento: linkPublico.orcamento,
        permissoes: linkPublico.permissoes as PermissaoLink[],
      };
  ...
  async atualizarLinkPublico(
    ...
    try {
      ...
      if (dados.permissoes !== undefined) {
        dadosAtualizacao.permissoes = dados.permissoes;
      }
  ...
  private async buscarLinksPorPermissao(orcamentoId: string): Promise<Record<string, number>> {
    ...
    links.forEach(link => {
      const permissoes = link.permissoes as string[];
  ...
  private transformarLinkPublico(link: any): LinkPublico {
    return {
      ...
      token: link.token,
      permissoes: link.permissoes as PermissaoLink[],
      data_expiracao: link.data_expiracao,
      max_visualizacoes: link.max_visualizacoes,
      senha: link.senha ? true : false, // Não retornar senha real
      ativo: link.ativo,
      data_criacao: link.data_criacao,
      visualizacoes: link.visualizacoes,
      orcamento: link.orcamento ? {
        id: link.orcamento.id,
        titulo: link.orcamento.titulo,
        cliente: link.orcamento.cliente,
      } : null,
      criado_por_usuario: link.criado_por_usuario ? {
        id: link.criado_por_usuario.id,
        nome: link.criado_por_usuario.nome,
        email: link.criado_por_usuario.email,
      } : null,
    };
  }
}
