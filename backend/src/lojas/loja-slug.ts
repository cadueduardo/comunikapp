...
/**
 * Gera slug a partir do nome; se inválido/reservado, usa prefixo loja-{idCurto}.
 */
export function suggestLojaSlugFromNome(nome: string, lojaId: string): string {
  const fromNome = normalizeLojaSlugCandidate(nome);
  if (isValidLojaSlug(fromNome)) return fromNome;
