...
export class OSPCPIntegrationService {
  ...
  async notificarInstanciaCriada(osId: string, workflowInstanciaId: string): Promise<void> {
    try {
      this.logger.log(`Instância de workflow criada para OS ${osId}: ${workflowInstanciaId}`);
  ...
  async notificarStatusAlterado(osId: string, status?: string): Promise<void> {
    try {
      ...
      this.logger.error(`Erro ao notificar mudança de status para OS ${osId}:`, error);
      throw error;
    }
  }
  ...
  async notificarInstanciaRemovida(osId: string): Promise<void> {
    try {
      ...
      this.logger.error(`Erro ao notificar remoção de instância para OS ${osId}:`, error);
      throw error;
    }
  }

  async notificarProgresso(osId: string, percentualConclusao: number): Promise<void> {
    try {
      this.logger.log(`Progresso atualizado para OS ${osId}: ${percentualConclusao}%`);
      ...
      this.logger.log(`Progresso da OS ${osId} atualizado: ${percentualConclusao}%`);
    } catch (error) {
  ...
  async notificarEtapaConcluida(osId: string, etapaNome: string): Promise<void> {
    try {
      ...
      this.logger.error(`Erro ao notificar conclusão de etapa para OS ${osId}:`, error);
      throw error;
    }
  }
  ...
  async notificarApontamento(osId: string, tipo: string): Promise<void> {
    try {
      ...
      this.logger.error(`Erro ao notificar apontamento para OS ${osId}:`, error);
      throw error;
    }
  }
