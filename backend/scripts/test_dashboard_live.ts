...
async function run() {
  try {
    console.log('🧪 Testando dashboard em tempo real após correções...');
    ...
    if (loginResponse.ok) {
      ...
      if (loginResult.access_token) {
        ...
        if (dashboardResponse.ok) {
          ...
          console.log('\n📈 ESTATÍSTICAS (O QUE INTERESSA):');
          console.log(`  - ENTRADAS: ${dashboardData.estatisticas?.entradas || 0}`);
          console.log(`  - SAÍDAS: ${dashboardData.estatisticas?.saias || 0} ← ESTE DEVE SER 4!`);
    ...
    if (e.message.includes('Failed to fetch')) {
      ...
      console.log('  - Verifique se está rodando na porta 4000');
    }
  }
}
