...
async function run() {
  try {
    console.log('🧪 Testando dashboard SEM cache (forçando nova requisição)...');
      ...
      if (loginResult.access_token) {
        ...
        const dashboardResponse = await fetch('http://localhost:4000/api/estoque/itens/dashboard', {
          ...
          headers: {
            ...
            'Pragma': 'no-cache',
            'Expires': '0',
            'If-None-Match': 'force-refresh-' + Date.now(),
        ...
        if (dashboardResponse.ok) {
          ...
          console.log(`  - SAÍDAS: ${dashboardData.estatisticas?.saidas || 0} ← DEVE SER 4!`);
          ...
          if (dashboardData.estatisticas?.saidas === 4) {
            console.log('\n🎉 PROBLEMA RESOLVIDO!');
            ...
            console.log(`  - Recebido: ${dashboardData.estatisticas?.saidas || 0} SAÍDAS`);
            console.log('\n🔍 Possíveis causas:');
