...
async function testarItens() {
  try {
    console.log('📦 Testando itens de estoque...');
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (jsonData.data && Array.isArray(jsonData.data)) {
            ...
            console.log(`   Total de itens: ${jsonData.data.length}`);
            jsonData.data.forEach((item, index) => {
              console.log(`   ${index + 1}. ${item.insumoNome} (${item.quantidadeAtual})`);
            });
          } else {
            console.log('\n❌ Itens não retornaram dados válidos');
          }
        } catch (e) {
