...
async function main() {
  ...
  const [locA, locB]: any[] = await prisma.$queryRawUnsafe(
    `SELECT id FROM localizacoes WHERE loja_id = ? AND codigo IN ('A1-01','B1-01') ORDER BY codigo`, lojaId
  )
  ...
  // 3) Lotes (idempotente)
  const item1: any[] = await prisma.$queryRawUnsafe('SELECT id FROM itens_estoque WHERE id = ? AND loja_id = ? LIMIT 1', 'seed-item-1', lojaId)
  const item2: any[] = await prisma.$queryRawUnsafe('SELECT id FROM itens_estoque WHERE id = ? AND loja_id = ? LIMIT 1', 'seed-item-2', lojaId)
  ...
  if (id2) {
    const exists2: any[] = await prisma.$queryRawUnsafe('SELECT id FROM estoque_lotes WHERE loja_id = ? AND numero_lote = ? LIMIT 1', lojaId, 'LT-TESTE-002')
    if (!exists2[0]) {
      await prisma.$executeRawUnsafe(
        `INSERT INTO estoque_lotes (id, estoque_id, numero_lote, data_fabricacao, data_validade, quantidade_lote, status, loja_id, criado_em)
  ...
  if (id2) {
    await prisma.$executeRawUnsafe(
      `INSERT INTO movimentacoes_estoque (id, loja_id, item_id, tipo, quantidade, observacoes, criado_em)
       VALUES ('seed-mov-ent-1', ?, ?, 'ENTRADA', 10, 'Entrada de teste', NOW())
       ON DUPLICATE KEY UPDATE quantidade=VALUES(quantidade), criado_em=NOW()`,
      lojaId, id2
    )
  }
  if (id1) {
  ...
  const item3: any[] = await prisma.$queryRawUnsafe('SELECT id FROM itens_estoque WHERE id = ? AND loja_id = ? LIMIT 1', 'seed-item-3', lojaId)
